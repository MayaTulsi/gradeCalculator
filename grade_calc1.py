...
def error_check(n, value, type, max, min, mod):     # various parameters used to test value for suitability
    try:
        truevalue = type(value)     # if value is of the expected type it is assigned to the variable 'truevalue'

        if truevalue > max or truevalue <= min:     # checks value is within specified size limitations (min, max)
            error_message(2, type, min, max)        # calls error function to print appropriate error message
...
def error_message(n,typ,min,max):   # error messages used in error_check function to nudge user to input correct value
    if n is 1:
